...
| `StackedContents#getBiggestCraftableStack` | `int getBiggestCraftableStack(RecipeHolder<?> recipe, @Nullable IntList)` | Max number of **sets** (crafts) the contents can supply. Fill it client-side like the server does: `player.getInventory().fillStackedContents(sc)` (36 main slots) **+** `((RecipeBookMenu<?,?>) menu).fillCraftSlotsStackedContents(sc)` (the loaded grid — both `CraftingMenu` and `InventoryMenu` extend `RecipeBookMenu`). Feature 1 caps its loop to this; feature 2 sizes its place-all cycles from it. |
| `placeAll` per-slot cap | `ServerPlaceRecipe#handleRecipeClicked` | `handlePlaceRecipe(id, recipe, true)` places `k = min(affordable, minIngredientMaxStack)` sets — i.e. **one stack per ingredient slot**, ≈64. So a single place-all + quick-move crafts ≤ ~64 sets; feature 2 loops `ceil(affordable / perCycleCap)` times (`RecipeClickPolicy#placeAllCycles`) to drain the rest. `perCycleCap` = the limiting ingredient's max stack, from `recipe.value().getIngredients()`. |
